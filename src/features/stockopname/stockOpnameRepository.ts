...
const SELECT_COLS = `p.id, p.name, p.sku, p.barcode, p.unit,
       COALESCE(os.stock, 0) AS system_stock`

/** Produk aktif + stok sistem pada outlet, untuk lembar opname. */
export function listOpnameProducts(outletId: number): OpnameProduct[] {
...
export function findOpnameProduct(code: string, outletId: number): OpnameProduct | null {
  const c = code.trim()
  if (!c) return null
  return (
    query<OpnameProduct>(
...
/**
 * Terapkan hasil opname: catat header + detail (stok sistem vs fisik + selisih)
 * lalu setel `outlet_stocks.stock` ke jumlah fisik. Satu transaksi SQL.
 */
export async function applyOpname(
  ...
  db.run('BEGIN')
  try {
    db.run('INSERT INTO stock_opnames (outlet_id, reference_number, note) VALUES (?, ?, ?)', [
      outletId,
      opnameRef(new Date()),
      note?.trim() || null,
    ])
    ...
    for (const c of items) {
      ...
      db.run(
        ...
        [outletId, c.productId, c.physical],
      )
    }
    db.run('COMMIT')
  } catch (err) {
...
/** Ringkasan opname terakhir pada outlet (untuk info di halaman). */
export function lastOpname(outletId: number): OpnameSummary | null {
  return (
    query<OpnameSummary>(
