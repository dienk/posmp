...
export function buildReceiptHtml(d: ReceiptData, c: ReceiptConfig): string {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>${d.invoice_number}</title>
    ...
    ${c.tagline ? `<div class="c" style="font-size:11px">${c.tagline}</div>` : ''}
    ${c.showAddress && d.outlet.address ? `<div class="c" style="font-size:11px">${d.outlet.address}</div>` : ''}
    ${c.showPhone && d.outlet.phone ? `<div class="c" style="font-size:11px">${d.outlet.phone}</div>` : ''}
    <hr>
    ...
    <hr>
    <div class="c">${c.footer}</div>
    ${c.note ? `<div class="c" style="font-size:10px;color:#666">${c.note}</div>` : ''}
    </body></html>`
}
...
export function ReceiptView({ data, config }: { data: ReceiptData; config: ReceiptConfig }) {
  ...
  return (
    ...
    >
      ...
      {config.tagline && <p className="text-center text-[11px]">{config.tagline}</p>}
      {config.showAddress && data.outlet.address && (
        <p className="text-center text-[11px]">{data.outlet.address}</p>
      )}
      {config.showPhone && data.outlet.phone && (
        <p className="text-center text-[11px]">{data.outlet.phone}</p>
      )}
      <Dashed />
      ...
      <Dashed />
      <p className="text-center">{config.footer}</p>
      {config.note && <p className="text-center text-[10px] text-ink-soft">{config.note}</p>}
    </div>
  )
}
