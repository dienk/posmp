...
| **M56** ✅ | **Data contact contoh**: seed mengisi modul Contact — **5 Pelanggan** (members: Andi Wijaya, Siti Rahmawati, Budi Hartono, Dewi Anggraini, Rudi Setiawan; poin & tier bervariasi), **4 Pemasok** (suppliers: CV Sumber Pangan, PT Kopi Nusantara, Toko Grosir Sejahtera, UD Segar Buah Malang), **3 Penjual** (sellers JSON `master_sellers`). Karyawan sudah ada 5 persona. DB baru langsung terisi; DB lama ditambah live via form. Terverifikasi tiap tab. |
| **M55** ✅ | **Zebra striping seluruh daftar data**: satu aturan base di `index.css` — `tbody tr:nth-child(even)` (tabel) & `[class*='divide-y'] > *:nth-child(even)` (list ul/li) diberi overlay `rgb(var(--c-ink)/0.05)`, **ikut tema** (gelapkan di tema terang, terangkan di gelap). Baris `:hover`/latar khusus (utility Tailwind) tetap menang; nav/dropdown tak pakai `divide-y` jadi tak ter-strip. Terverifikasi: tabel Produk & list Bundling ber-zebra. |
| **M54** ✅ | **Pencarian gabung barcode di Kartu Stock**: combobox pencarian (M52) kini juga mencocokkan **barcode** selain nama & SKU (`listStockCardProducts` menyertakan `barcode`; placeholder "Cari nama / SKU / barcode…"). Terverifikasi: ketik `8991234000038` → Choco Lava. |
| **M53** ✅ | **Filter stok ≤ minimum di Kartu Stock**: tombol **⚠️ Stok Menipis · N** di header (N = jumlah produk stok ≤ min). Aktif → pemilih produk hanya menampilkan produk dengan **stok total ≤ stok minimum**; tiap hasil menampilkan `stok / min` (stok merah bila menipis) + badge "≤ min". `listStockCardProducts` kini menyertakan `min_stock`. Terverifikasi: set min Choco Lava 30 (stok 25) → badge "· 1", filter menampilkan hanya Choco Lava. |
| **M52** ✅ | **Pencarian nama/SKU di Kartu Stock**: pemilih produk `<select>` diganti **combobox pencarian** (`StockCardPage`) — ketik untuk filter produk berdasarkan **nama ATAU SKU**, dropdown menampilkan nama + SKU + stok; pilih → kartu dimuat. Input di-bind ke kata kunci (nilai selalu = search) + produk terpilih tampil sebagai placeholder, jadi tak ada race saat mengetik. Input scan barcode tetap ada. Terverifikasi: "cap"→Cappuccino, "teh"→Es Teh Manis+Teh Tarik, "MNM-001"→Es Teh Manis. |
| **M51** ✅ | **Filter gudang di menu Bundling**: dropdown **🏬 Semua Gudang / <gudang>** di header Bundling. `listBundles(outletId, warehouseId?)` — bila gudang dipilih, stok komponen (→ ketersediaan paket "Bisa dijual") dihitung dari gudang itu saja; tanpa pilih = total lintas gudang. Terverifikasi: Semua Gudang → 44/40/49/27/33/35; Gudang Sementara (komponen kosong) → semua 0. |
